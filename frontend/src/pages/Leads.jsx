    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
            <Users className="w-6 h-6 text-brand-600" /> Inbound Leads & Prospecting
          </h1>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 flex flex-wrap items-center justify-between gap-4 shadow-sm">
        <div className="flex-1 min-w-[260px] relative">
          ...
          <input
            ...
            className="w-full bg-slate-50 border border-slate-300 rounded-lg pl-9 pr-4 py-2 text-xs text-slate-900 focus:outline-none focus:border-brand-500"
          />
        </div>
            ...
            >
              <div className="space-y-2">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="text-sm font-bold text-slate-900">{lead.firstName} {lead.lastName}</h3>
                    <p className="text-xs text-brand-700 font-semibold">{lead.company}</p>
                  </div>
                ...
                {lead.notes && (
                  <p className="text-[11px] text-slate-700 bg-slate-50 p-2.5 rounded-lg border border-slate-200 line-clamp-2">
                    {lead.notes}
                  </p>
                )}
              </div>
              ...
              <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
                ...
                {lead.status !== 'CONVERTED' ? (
                  <button
                    ...
                    className="px-3 py-1.5 bg-brand-600 hover:bg-brand-700 text-white font-extrabold text-[11px] rounded-lg shadow-sm flex items-center gap-1 transition-all"
                  >
      ...
      <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} title="Create New Inbound Lead">
        <form onSubmit={handleCreateLead} className="space-y-4 text-xs">
          ...
          <div>
            <label className="block text-slate-700 font-semibold mb-1">Notes & Scope</label>
            <textarea
              rows={2}
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-200">
            <button type="button" onClick={() => setIsCreateModalOpen(false)} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg font-semibold">Cancel</button>
            <button type="submit" disabled={submittingCreate} className="px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-lg shadow-sm">
      ...
      <Modal isOpen={!!convertTargetLead} onClose={() => setConvertTargetLead(null)} title="1-Click Lead Conversion">
        <form onSubmit={handleConvertLead} className="space-y-4 text-xs">
          <p className="text-slate-700">
            Converting lead <strong className="text-slate-900">{convertTargetLead?.firstName} {convertTargetLead?.lastName} ({convertTargetLead?.company})</strong> into an Account, Contact, and Governed Deal.
          </p>

          <div>
            <label className="block text-slate-700 font-semibold mb-1">Deal Title *</label>
            <input
              type="text"
              required
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-200">
            <button type="button" onClick={() => setConvertTargetLead(null)} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg font-semibold">Cancel</button>
            <button type="submit" disabled={submittingConvert} className="px-5 py-2 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-lg shadow-sm">
